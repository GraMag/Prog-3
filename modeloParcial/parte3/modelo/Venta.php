...
class Venta implements JsonSerializable{ 
    ...
    public static function devolverHelado($post, $file){
        $listaVentas = Venta::mapper(Archivo::leer(Venta::$jsonPath));
        ...
        foreach ($listaVentas as $ventaActual) {
            if($ventaActual->getNumeroDePedido() == $post["numeroDePedido"]){
                ...
                $listaCupones = Cupon::mapper(Archivo::leer($cupon->getPath()));
                $listaDevolucion = Devolucion::mapper(Archivo::leer($devolucion->getPath()));  
                
                $listaCupones[] = $cupon;
                $listaDevolucion[] = $devolucion;

                Archivo::guardar($cupon->getPath(), $listaCupones);
                Archivo::guardar($devolucion->getPath(), $listaDevolucion);
